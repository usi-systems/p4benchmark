...
class BenchmarkHeaderAddition(P4Benchmark):

    def __init__(self, nb_header, nb_field, offer_load):
        parent_dir = 'result/add_header'
        directory = '{0}/{1}/{2}'.format(parent_dir, nb_header, offer_load)
        super(BenchmarkHeaderAddition, self).__init__(parent_dir, directory, offer_load)
    ...
    def compile_p4_program(self):
        ret = benchmark_modification(self.nb_header, self.nb_field, 'add')
        assert (ret == True)
        prog = 'main'
...
def run(nb_headers=2, nb_fields=2, step=2):
    while(nb_headers <= 40):
        offer_load = 100000
        p = BenchmarkHeaderAddition(nb_headers, nb_fields, offer_load)
        # compile
